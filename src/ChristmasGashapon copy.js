...
export default function ChristmasGashapon() {
  ...
  const handleUpload = () => {
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        // สร้าง balls จาก jsonData
        const initialBalls = jsonData.map(() => ({
          top: Math.random() * 400,
          left: Math.random() * 400,
          velocityX: 0,
          velocityY: 0,
        }));
        setBalls(initialBalls); // <- สำคัญ!
      };
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      ...
      try {
        ...
        const initialBalls = validEmployees.map(() => ({
          top: Math.random() * 400,
          left: Math.random() * 400,
          velocityX: 0,
          velocityY: 0,
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setBalls((prev) =>
        prev.map((ball) => {
          ...
          if (top > 440) { top = 440; velocityY *= -0.5; }
          if (left < 0) { left = 0; velocityX *= -1; }
          if (left > 440) { left = 440; velocityX *= -1; }
    ...
    setSnowActive(true); // start snowfall
    return () => clearInterval(interval);
  }, []);

  // Send winner email to API
  ...
  const drawWinner = () => {
    ...
    setTimeout(() => {
      ...
      setWinnerName(winner.fullname);
      setWinnerEmail(winner.email);
      setShowWinner(true);

      // Send email
      sendWinnerEmail(winner.email);
  ...
  return (
    <div className="scene-wrapper">
      ...
      <div className={`sidebar ${isSidebarVisible ? 'show' : 'hide'}`}>
        ...
        <label htmlFor="chooseFile" className="custom-file-upload">Choose File</label>
        <input type="file" id="chooseFile" accept=".xlsx" onChange={handleFileChange} />
        <button className="upload-btn" onClick={handleUpload}>Upload</button>
        <h5>จำนวนพนักงาน {employees.length} คน</h5>
      ...
      <div className="gashapon-wrapper">
        ...
        <p style={{ marginTop: "10px" }}>
          {winnerName && showWinner ? `Winner: ${winnerName}` : ""}
        </p>

        {/* Snowfall */}
        <Snowfall active={snowActive}>
          <div className="snow-decoration snowman">⛄</div>
          <div className="snow-decoration reindeer">🦌</div>
        </Snowfall>
